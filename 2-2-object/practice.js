...
for (let list of userInfo.userList) {
  passwordList.push(list.password);
}
...
while (true) {
  ...
  if (accountList.includes(inputAccount)) {
    ...
    if(inputPassword === userInfo.userList[accountList.indexOf(inputAccount)].password) {
      alert(`${userInfo.userList[accountList.indexOf(inputAccount)].username}님 로그인 성공!`);
      break;
    } else {
      alert('비밀번호가 틀렸습니다!');
    }
